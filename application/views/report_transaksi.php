			 			...
			 			<fieldset>
			 				...
			 				<i class="fa fa-print"></i>
			 				  ...
			 				  <div class="form-horizontal">
			 				    <form name="print_report" id="print_report">
	...
	<script type="text/javascript">
		$(function(){
			$('#table_rekap').DataTable({
						"ajax": {
			            "url": "<?= site_url('report/ajax_list_rekaptransaksi')?>",
			            "type": "GET"
			        }
			});
			 $('#tgl').datepicker({
			...
			$('#print').on('click',function(){
				...
				if(tanggal_transaksi == ''){
					...
					var preview = $("#previewpdf");
						...
						$.ajax({
									url : "<?= site_url('report/exportpdf_transaksi_pertanggal')?>/"+tanggal_transaksi,
						            type: "GET",
						            dataType: "HTML",
						            success: function(data)
						            {
						               $('#previewpdf').attr('src','<?= site_url('report/exportpdf_transaksi_pertanggal')?>/'+tanggal_transaksi);
